...
import { practiceModes, presetTopics, type PracticeMode, type PracticeTopic } from "../data/presetExercises";
  ...
  return (
    <>
      ...
      <div className="option-strip" role="group" aria-label="内容来源">
        ...
        <button type="button" disabled>随机练习 <small>未开放</small></button>
...
function TopicQuestions({ topic, index }: { topic: PracticeTopic; index: number }) {
  ...
  return (
    <section className="topic-section" aria-labelledby={`topic-${topic.id}`}>
      ...
      <div className="topic-content">
        <div className="option-strip topic-modes" role="group" aria-label={`${topic.title}的训练方式`}>
          {practiceModes.map((mode) => (
            <button key={mode.id} type="button"
              className={selectedMode === mode.id ? "option-current" : undefined}
              aria-pressed={selectedMode === mode.id}
              aria-controls={`questions-${topic.id}`}
              disabled={!mode.available}
              onClick={() => setSelectedMode(mode.id)}>
              {mode.label}{!mode.available && <small> 未开放</small>}
            </button>
          ))}
        </div>
            ...
            <li key={question.id}>
              <Link className="question-link" to={`/practice/${question.id}`}>
                <div className="question-copy">
                  ...
                  <span className="question-meta">{question.exercise.measures.length} 小节</span>
                </div>
