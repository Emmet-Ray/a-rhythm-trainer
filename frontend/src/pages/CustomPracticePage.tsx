    ...
    return (
      <>
        ...
        <Suspense fallback={<p className="loading-message" role="status">正在加载编辑器…</p>}>
          ...
          <CustomExerciseEditor key={selectedMode.id} />
        </Suspense>
      </>
    );
  }
      ...
      <section className="custom-mode-selection" aria-labelledby="custom-mode-heading">
        ...
        <ul className="question-list">
          {customModes.map((item) => (
            <li key={item.id}>
              <Link className="question-link" to={`/custom/${item.id}/new`}>
                <h3>{item.label}</h3>
                <span className="question-action">新建练习 <span aria-hidden="true">→</span></span>
              </Link>
            </li>
          ))}
