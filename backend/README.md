...
账号自定义练习已接通前端的保存、分页列表和练习入口，本地练习不会自动导入。保存上限为名称 100 字符、64 个完整 4/4 小节；三个接口都要求有效会话，并强制按用户范围执行。列表只返回摘要，完整节奏通过单题接口读取。具体存储约定见 [db/custom_exercises.py](db/custom_exercises.py)，接口示例见 [联调说明](../联调.md)。
